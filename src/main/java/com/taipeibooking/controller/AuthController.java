    ...
    public ResponseEntity<?> registerUser(@RequestBody RegisterRequest request) {
        ...
        try {
            ...
            if (registeredUser != null) {
                ...
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", true, "message", "註冊失敗，此 Email 已被註冊或發生其他錯誤"));
            }
    ...
    public ResponseEntity<?> loginUser(@RequestBody LoginRequest request) {
        ...
        try {
            ...
            User user = userRepository.findByEmail(email)
                .orElseThrow(() -> new RuntimeException("已認證的使用者在資料庫中找不到: " + email));
    ...
    public ResponseEntity<?> getUserAuth() {
        ...
        if (authentication != null && authentication.isAuthenticated() && !"anonymousUser".equals(authentication.getPrincipal())) {
            ...
            if (userOptional.isPresent()) {
                ...
                logger.error("錯誤：已認證的使用者在資料庫中找不到: {}", email);
